ALTER TABLE dispatch_flow_template ADD COLUMN active_published_version_id BIGINT NULL;

ALTER TABLE dispatches
  ADD COLUMN workflow_version_id BIGINT NULL,
  ADD COLUMN pod_required BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN pod_submitted BOOLEAN NOT NULL DEFAULT FALSE,
  ADD COLUMN pod_submitted_at TIMESTAMP NULL,
  ADD COLUMN pod_verified BOOLEAN NOT NULL DEFAULT FALSE;

CREATE INDEX idx_dispatch_workflow_version_id ON dispatches (workflow_version_id);
...
INSERT INTO dispatch_flow_template_version (
  template_id, version_no, version_label, status, active_published, source_updated_at, notes, created_by, published_at
)
SELECT t.id, 1, 'v1', 'PUBLISHED', TRUE, t.updated_at, 'Initial snapshot from editable draft tables', t.updated_by, CURRENT_TIMESTAMP
FROM dispatch_flow_template t
WHERE NOT EXISTS (
  SELECT 1 FROM dispatch_flow_template_version v WHERE v.template_id = t.id
);

INSERT INTO dispatch_flow_transition_rule_version (
  template_version_id, source_rule_id, from_status, to_status, enabled, priority, requires_confirmation, requires_input, validation_message, metadata_json
)
SELECT v.id, r.id, r.from_status, r.to_status, r.enabled, r.priority, r.requires_confirmation, r.requires_input, r.validation_message, r.metadata_json
FROM dispatch_flow_transition_rule r
JOIN dispatch_flow_template_version v ON v.template_id = r.template_id AND v.version_no = 1
WHERE NOT EXISTS (
  SELECT 1 FROM dispatch_flow_transition_rule_version rv
  WHERE rv.template_version_id = v.id AND rv.from_status = r.from_status AND rv.to_status = r.to_status
);

INSERT INTO dispatch_flow_transition_actor_version (
  transition_rule_version_id, actor_type, can_execute
)
SELECT rv.id, a.actor_type, a.can_execute
FROM dispatch_flow_transition_actor a
JOIN dispatch_flow_transition_rule r ON r.id = a.transition_rule_id
JOIN dispatch_flow_template_version v ON v.template_id = r.template_id AND v.version_no = 1
JOIN dispatch_flow_transition_rule_version rv ON rv.template_version_id = v.id AND rv.from_status = r.from_status AND rv.to_status = r.to_status
WHERE NOT EXISTS (
  SELECT 1 FROM dispatch_flow_transition_actor_version av
  WHERE av.transition_rule_version_id = rv.id AND av.actor_type = a.actor_type
);

UPDATE dispatch_flow_template t
JOIN dispatch_flow_template_version v ON v.template_id = t.id AND v.active_published = TRUE
SET t.active_published_version_id = v.id
WHERE t.active_published_version_id IS NULL;

UPDATE dispatches d
LEFT JOIN dispatch_flow_template t ON UPPER(t.code) = UPPER(COALESCE(NULLIF(TRIM(d.loading_type_code), ''), 'GENERAL'))
LEFT JOIN dispatch_flow_template tg ON tg.code = 'GENERAL'
LEFT JOIN dispatch_flow_template_version v ON v.id = COALESCE(t.active_published_version_id, tg.active_published_version_id)
