package com.svtrucking.logistics.model;

import jakarta.persistence.*;
import lombok.AllArgsConstructor;
import lombok.Builder;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;

@Getter
@Setter
@Entity
@Table(
    name = "loading_pallet_items",
    indexes = {
        @Index(name = "idx_loading_pallet_session", columnList = "loading_session_id")
    }
)
@NoArgsConstructor
@AllArgsConstructor
@Builder
public class LoadingPalletItem {

  @Id
  @GeneratedValue(strategy = GenerationType.IDENTITY)
  private Long id;

  @ManyToOne(fetch = FetchType.LAZY, optional = false)
  @JoinColumn(name = "loading_session_id", nullable = false)
  private LoadingSession loadingSession;

  @Column(name = "item_description", length = 255, nullable = false)
  private String itemDescription;

  @Column(name = "pallet_tag", length = 64)
  private String palletTag;

  @Column(name = "quantity", nullable = false)
  private Integer quantity;

  @Column(name = "unit", length = 32)
  private String unit;

  @Column(name = "condition_note", length = 255)
  private String conditionNote;

  @Column(name = "verified_ok", nullable = false)
  private boolean verifiedOk;
}
