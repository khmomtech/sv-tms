...
public class AuthorizationService {
  ...
  private static final Set<String> EXPLICIT_ONLY_PERMISSIONS = Arrays.stream(
          new String[] {
            PermissionNames.DISPATCH_FLOW_MANAGE,
            PermissionNames.DISPATCH_STATUS_OVERRIDE,
            PermissionNames.DISPATCH_STATUS_MANUAL_UPDATE
          })
  ...
  public boolean hasPermission(@Nullable String permissionName) {
    ...
    String normalizedPermission = permissionName.trim().toLowerCase();
    boolean explicitOnly = EXPLICIT_ONLY_PERMISSIONS.contains(normalizedPermission);
    ...
    if (auth != null && auth.isAuthenticated()) {
      for (GrantedAuthority ga : auth.getAuthorities()) {
        ...
        if (!explicitOnly && a.equalsIgnoreCase(PermissionNames.ALL_FUNCTIONS)) return true;
      }
    }
    ...
    if (!explicitOnly && effectivePermissions.stream()
