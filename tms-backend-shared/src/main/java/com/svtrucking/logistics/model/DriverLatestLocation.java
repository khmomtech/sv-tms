// model/DriverLatestLocation.java
...
public class DriverLatestLocation {
  ...
  public void prePersist() {
    ...
    if (Double.isNaN(latitude) || Double.isInfinite(latitude)) latitude = 0d;
    if (Double.isNaN(longitude) || Double.isInfinite(longitude)) longitude = 0d;

    if (latitude > 90d) latitude = 90d;
    else if (latitude < -90d) latitude = -90d;
    if (longitude > 180d) longitude = 180d;
    else if (longitude < -180d) longitude = -180d;

    if (this.lastSeen == null) this.lastSeen = new Timestamp(now);
    if (this.isOnline == null) this.isOnline = Boolean.TRUE;
  }
  ...
  public boolean isStale(long thresholdMs) {
    if (this.lastSeen == null) return true;
    long age = System.currentTimeMillis() - this.lastSeen.getTime();
