 ...
 */
async function waitForBackend(apiUrl, timeout) {
    ...
    throw new Error(`Backend not available after ${timeout}ms. Last error: ${lastError?.message || 'Unknown'}`);
}
/**
 * Health check for frontend application
 * Verifies frontend is serving content
 */
async function waitForFrontend(frontendUrl, timeout) {
    const startTime = Date.now();
    while (Date.now() - startTime < timeout) {
        try {
 ...
 */
async function setupDatabase(apiUrl) {
    try {
 ...
 */
async function setupTestUsers(apiUrl) {
    try {
 ...
 */
async function saveAuthState(frontendUrl, apiUrl) {
    ...
    try {
        ...
        await page.fill('input[type="email"], input[name="username"], input[formControlName="username"]', username);
        await page.fill('input[type="password"], input[name="password"], input[formControlName="password"]', password);
