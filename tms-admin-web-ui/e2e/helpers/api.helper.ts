 ...
 */
export async function getDrivers(page: Page) {
 ...
 */
export async function getVehicles(page: Page) {
 ...
 */
export async function getTransportOrders(page: Page) {
 ...
 * Create test data
 */
export async function createDriver(page: Page, driverData: any) {
  const response = await apiRequest(page, '/api/drivers', {
    method: 'POST',
    body: JSON.stringify(driverData),
  });
  if (!response.ok) throw new Error(`Failed to create driver: ${response.status}`);
  return response.json();
}
 ...
 * Delete test data
 */
export async function deleteDriver(page: Page, driverId: number) {
  const response = await apiRequest(page, `/api/drivers/${driverId}`, {
    method: 'DELETE',
  });
  if (!response.ok) throw new Error(`Failed to delete driver: ${response.status}`);
  return response.ok;
}
