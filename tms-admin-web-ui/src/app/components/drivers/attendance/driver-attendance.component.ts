...
import { DriverFilter } from '../../../services/driver.service';
import { ConfirmService } from '../../../services/confirm.service';

interface AttendanceRecord {
  id?: number;
  driverId: number;
  driverName?: string; // added from backend DTO for month-wide view
  truckPlateNo?: string; // added from backend DTO for month-wide view
...
export class DriverAttendanceComponent implements OnInit, OnDestroy {
  ...
  // NOTE: This component intentionally supports ONLY Year + Month filtering.
  // Any previous from/to date range UI has been removed per requirement; the
  // 'Filter' button triggers a reload using these two fields.
  // Filters
  ...
  // Month-wide pagination (permission-only always enforced now; toggle removed)
  monthPage = 0;
  monthSize = 20;
  monthTotalPages = 0;
  ...
  // Driver search (typeahead) - top filter
  ...
  // Removed advanced filters and buttons: keep only autocomplete dropdown
  ...
  form = {
    ...
    driverId: new FormControl<number | null>(null, { nonNullable: false, validators: [] }),
    date: new FormControl<string>('', { nonNullable: false, validators: [Validators.required] }),
    endDate: new FormControl<string>('', { nonNullable: false }), // optional for range bulk create
  ...
  ngOnInit(): void {
    // Auto-search on input change (debounced), no explicit Search/Advanced UI
  ...
  loadAttendance(): void {
    ...
    if (!driverId) {
      // No driver selected: load all drivers' permission records for the month (permission-only enforced)
      this.driverService
    ...
    // Driver selected: load per-driver list and summary (month based today)
    // For range mode with driver selected, keep existing month summary API (optional future enhancement)
    forkJoin({
  ...
  // Pagination controls (month-wide)
  ...
  // (Removed permission-only toggle; permission filter always applied)
  ...
  // ── Top autocomplete handlers ─────────────────────────────────────────────
  onTopFocus(): void {
    // Keep dropdown visibility; do not trigger network calls automatically
    this.showDropdownTop = this.driverResultsTop.length > 0;
  }

  onTopBlur(): void {
    // Delay closing so mousedown on option can fire
  ...
  openEdit(rec: AttendanceRecord): void {
    ...
    this.form.endDate.setValue(''); // editing single record; keep range empty
  ...
  save(): void {
    ...
    // If endDate provided AND not editing existing record -> bulk range creation (permission only)
    if (!this.isEditing && endDate) {
      // Basic validation: endDate >= date
      if (endDate < date) {
