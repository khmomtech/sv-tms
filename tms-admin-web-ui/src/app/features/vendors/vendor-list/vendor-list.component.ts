...
export class PartnerListComponent {
  ...
  // Remainder identical logic to existing PartnerListComponent
  // Copy-pasted implementation preserved for consistency
  ...
  async deactivatePartner(partner: PartnerCompany): Promise<void> {
    ...
    const ok = await this.confirm.confirm(
      `Are you sure you want to deactivate ${partner.companyName}?`,
    );
    if (!ok) return;
  ...
  submitForm(): void {
    ...
    const raw = this.form.getRawValue() as PartnerCompany;
    const payload: PartnerCompany = {
      ...raw,
      companyName: raw.companyName?.trim() || '',
      partnershipType: raw.partnershipType,
      status: raw.status,
      commissionRate: raw.commissionRate == null ? undefined : Number(raw.commissionRate),
      creditLimit: raw.creditLimit == null ? undefined : Number(raw.creditLimit),
    };
    ...
    req$.subscribe({
      next: (result) => {
        if (!result || !result.id) {
          this.toast('Save response invalid');
      ...
      error: (err) => {
        ...
        const msg = err?.error?.message || 'Failed to save vendor';
        this.toast(msg);
        this.saving.set(false);
      },
    });
  ...
  ngAfterViewInit(): void {
    // Async validation for business license uniqueness
    const licenseControl = this.form.get('businessLicense');
    if (licenseControl) {
      licenseControl.valueChanges
        .pipe(debounceTime(400), distinctUntilChanged())
        .subscribe((val) => {
          if (!val) {
            licenseControl.setErrors(null);
            return;
          }
          this.partnerService.checkBusinessLicenseExists(val).subscribe((exists) => {
            if (exists) {
              licenseControl.setErrors({ licenseTaken: true });
            } else {
              // Preserve other potential errors
              const errors = licenseControl.errors;
              if (errors) {
                delete errors['licenseTaken'];
                if (Object.keys(errors).length === 0) {
                  licenseControl.setErrors(null);
                } else {
                  licenseControl.setErrors(errors);
                }
              } else {
                licenseControl.setErrors(null);
              }
