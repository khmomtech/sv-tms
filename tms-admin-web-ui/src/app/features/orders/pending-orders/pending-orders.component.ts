...
export class PendingOrdersComponent implements OnInit {
  ...
  get totalPagesDisplay(): number {
    // If backend-controlled pagination is used this will be approximate; show 1 when pending-only
    return 1;
  }

  ngOnInit(): void {
    // Ensure the list receives the status query param so it pre-filters to PENDING
    this.router.navigate([], {
      relativeTo: this.route,
      queryParams: { status: 'pending', pendingOnly: 'true' },
