...
import type { ApiResponse, SafetyCategory, SafetyMasterItem } from '../models/safety-master.model';

@Injectable({ providedIn: 'root' })
export class SafetyMasterDataService {
  private readonly http = inject(HttpClient);
  private readonly baseUrl = `${environment.apiBaseUrl}/admin/safety-master`;
  ...
  deactivateCategory(id: number): Observable<ApiResponse<SafetyCategory>> {
    return this.http.delete<ApiResponse<SafetyCategory>>(`${this.baseUrl}/categories/${id}`);
  }
  ...
  importFromExcel(file: File): Observable<ApiResponse<any>> {
    ...
    return this.http.post<ApiResponse<any>>(`${this.baseUrl}/import`, form);
  }
  ...
  deactivateItem(id: number): Observable<ApiResponse<SafetyMasterItem>> {
    return this.http.delete<ApiResponse<SafetyMasterItem>>(`${this.baseUrl}/items/${id}`);
  }
}
