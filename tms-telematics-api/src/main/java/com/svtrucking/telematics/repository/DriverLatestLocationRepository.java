...
public interface DriverLatestLocationRepository extends JpaRepository<DriverLatestLocation, Long> {
    ...
    @Query(value = """
            ...
            VALUES (
                :driverId, :lat, :lng, :speed, :heading, :dispatchId, CURRENT_TIMESTAMP,
                :batteryLevel, :source, :locationName, :isOnline,
                :accuracyMeters, :locationSource, :netType, :appVersionCode
            )
            ON CONFLICT (driver_id) DO UPDATE
            SET latitude = COALESCE(EXCLUDED.latitude, driver_latest_location.latitude),
                longitude = COALESCE(EXCLUDED.longitude, driver_latest_location.longitude),
                speed = COALESCE(EXCLUDED.speed, driver_latest_location.speed),
                heading = COALESCE(EXCLUDED.heading, driver_latest_location.heading),
                dispatch_id = COALESCE(EXCLUDED.dispatch_id, driver_latest_location.dispatch_id),
                last_seen = CURRENT_TIMESTAMP,
                battery_level = COALESCE(EXCLUDED.battery_level, driver_latest_location.battery_level),
                source = COALESCE(EXCLUDED.source, driver_latest_location.source),
                location_name = COALESCE(EXCLUDED.location_name, driver_latest_location.location_name),
                is_online = COALESCE(EXCLUDED.is_online, driver_latest_location.is_online, TRUE),
                accuracy_meters = COALESCE(EXCLUDED.accuracy_meters, driver_latest_location.accuracy_meters),
                location_source = COALESCE(EXCLUDED.location_source, driver_latest_location.location_source),
                net_type = COALESCE(EXCLUDED.net_type, driver_latest_location.net_type),
                version = COALESCE(EXCLUDED.version, driver_latest_location.version)
            """, nativeQuery = true)
    int upsertLatest(
            @Param("driverId") Long driverId,
